...
function handleMarketplacePurchase (app, context) {
  const { action, marketplace_purchase: { account, plan }, previous_marketplace_purchase: previous } = context.payload
  ...
  const planEmoji = plan.name === 'Free' ? '🆓' : plan.name === 'Pro' ? '💵' : '💰'

  log.info(`${changeEmoji}${planEmoji} ${account.type} ${account.login} ${action === 'changed' ? 'changed to' : action} ${plan.name}`)
}
