...
export default async function handlePullRequestChange({
  ...
  try {
    ...
    // merge_group events always pass — the PR was already checked before
    // entering the merge queue, and the payload has no pull_request data.
    // Draft PRs always override to WIP status, skip further checks.
    const newStatus = mergeGroup
      ? { wip: false }
      : pr.draft
        ? { wip: true, draft: true }
        : plan === "free"
    ...
    const shortUrl = mergeGroup
      ? `${repo.full_name}@${mergeGroup.head_sha.slice(0, 7)}`
    ...
    log = getLogChild({
      log,
      eventName: mergeGroup ? "merge_group" : "pull_request",
      action,
      plan,
      newStatus,
      repo,
