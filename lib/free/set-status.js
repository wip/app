...
export default function setStatusFree(newStatus, { octokit, payload }) {
  const pullRequest = payload.pull_request || {};
  const headSha = payload.merge_group
  ...
  const checkOptions = {
    ...
    output: {
      title: newStatus.match
        ...
        : "Ready for review",
      summary: newStatus.match
        ? `The title "${pullRequest.title}" contains "${newStatus.match}".`
