...
import PRO_PLAN_FOR_FREE from "../../pro-plan-for-free.js";
...
export default async function setStatusPro(newStatus, { octokit, payload }) {
  const {
    repository,
    pull_request: pullRequest,
    merge_group: mergeGroup,
  } = payload;
  const headSha = mergeGroup ? mergeGroup.head_sha : pullRequest.head.sha;
...
<table>
  ...
  if (PRO_PLAN_FOR_FREE.includes(repository.owner.login.toLowerCase())) {
    checkOptions.output.summary += `
    ### 🆓💸 The account ${repository.owner.login} is [enabled for the pro plan for free](https://github.com/wip/app/blob/main/pro-plan-for-free.js)
  ...
  return octokit.rest.checks.create(getRepo(payload, checkOptions));
}
