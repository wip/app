...
test('new pull request with "Test" title', async function (t) {
  ...
  t.is(this.githubMock.repos.createStatus.lastCall.arg.target_url, 'https://github.com/organizations/wip/settings/installations/1/permissions/update')
...
test('new pull request with "Test" title from user', async function (t) {
  await this.app.receive(require('./events/new-pull-request-with-test-title-from-user.json'))

  // check setting new status
  t.is(this.githubMock.repos.createStatus.callCount, 1)
  t.is(this.githubMock.repos.createStatus.lastCall.arg.target_url, 'https://github.com/settings/installations/1/permissions/update')
